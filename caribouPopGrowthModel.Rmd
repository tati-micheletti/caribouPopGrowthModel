...
params <- list(
  "caribouPopGrowthModel" = list(
    "polygonsOfInterest" = c("Dehcho North_v2", "Dehcho South_v2", 
                             "Hay River Lowlands", "GSA North", "GSA South"),
    ".growthInterval" = 10
  )
)
...
studyArea <- Cache(prepInputs, url = "https://drive.google.com/file/d/1RPfDeHujm-rUHGjmVs6oYjLKOKDF0x09/view?usp=sharing",
                                destinationPath = file.path(Paths$modulePath, "data"),
...
listSACaribou <- list("metaHerds" =  Cache(prepInputs, 
                       ...
                       userTags = c("inputs", 
                                    "outFun:Cache",
                                    "step:prepHeardsPoly")))
listSACaribou$metaHerds <- projectInputs(listSACaribou$metaHerds, targetCRS = crs(rasterToMatch))
######################## END ERASE!
...
caribous <- simInitAndSpades(times = times, 
                             ...
                             objects = list("listSACaribou" = listSACaribou))
```
