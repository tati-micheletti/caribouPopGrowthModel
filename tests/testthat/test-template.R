...
#    `functionName` is a function's name in your module (e.g., `caribouPopGrowthModelEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/tmichele/GitHub/caribouPopGrowthModel/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("caribouPopGrowthModel")
  path <- list(modulePath = "C:/Users/tmichele/GitHub",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    caribouPopGrowthModel = list(.saveInitialTime = NA)
  )
  ...
  if (exists("caribouPopGrowthModelEvent1", envir = .GlobalEnv)) {
    simOutput <- caribouPopGrowthModelEvent1(mySim)
  } else {
  ...
  if (exists("caribouPopGrowthModelEvent2", envir = .GlobalEnv)) {
    simOutput <- caribouPopGrowthModelEvent2(mySim)
  } else {
